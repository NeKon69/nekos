...
using nekos::interrupts::discoverMADT;
using nekos::interrupts::findIOAPIC;
using nekos::interrupts::IOAPIC;
using nekos::interrupts::MADTIOAPIC;
...
APIC::APIC(MB2Tag *TagList) {
  ...
  setupSIV();
  // Run only with debug.
  nekosAssert((readAPICID(), true));
  nekosAssert((readAPICMetadata(), true));
...
void LVTErrorHandler(const Registers *Regs [[maybe_unused]]) {
  uint32_t error = APIC::LAPICAddress[0x280 / 4];
...
void APIC::setupIOAPIC(MB2Tag *TagList) {
  uint32_t *MADT = discoverMADT(TagList);
  MADTIOAPIC *IOAPICRecord = findIOAPIC(MADT);
  IOAPIC IOApic(IOAPICRecord->Address);
  nekosAssert(IOApic.readVersion() != 0);
}
